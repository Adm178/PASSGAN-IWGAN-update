...
class WGANGP:
    ...
    def train(self, dataset):
        ...
        def string_to_vector(string, length):
            encoded = tf.strings.unicode_decode(string, 'utf-8')  # Кодируем строку в UTF-8
            if tf.shape(encoded)[0] >= length:
                print("-----1", encoded)
                encoded = tf.slice(encoded, [0], [self.seq_len])
                print("-----2", encoded)
            padding_value = tf.maximum(length - tf.shape(encoded)[0], 0)
            vector = tf.pad(encoded, paddings=[[0, padding_value]])[:length]


            #vector = tf.pad(encoded, paddings=[[0, length - tf.shape(encoded)[0]]])[:length]
            return vector

        global batch_c
        batch_c = [self.batch_size, 1, self.seq_len]
        for epoch in tf.range(self.epochs):
            ...
            for iteration, batch in zip(range(self.iterations), dataset):
                for _ in tf.range(self.n_critic):
                    self.text = batch['password']#[:self.batch_size]
                    #print("\n\n-----------", tf.shape(self.text))
                    #print("\n\n-----------",self.text)
                    batch_vectors = tf.map_fn(lambda x: string_to_vector(x, self.seq_len), self.text, dtype=tf.int32)
                    batch_vectors = tf.cast(batch_vectors, tf.float32)
                    batch_vectors = tf.expand_dims(batch_vectors, -1)
                    #for vector in zip(batch_vectors):
                    #    print("\n---------", tf.shape(vector))
                    real = tf.reshape(batch_vectors, batch_c)
                    #print("\n---------", tf.shape(real))
                    #for vector in zip(real):
                    #    print("\n---------", tf.shape(vector))
    ...
    @tf.function
    def train_g(self):
        z = tf.random.normal(batch_c, dtype=tf.dtypes.float32)
    ...
    def gradient_penalty(self, f, real, fake):
        alpha = random.uniform(batch_c, 0., 1.)
        diff = fake - real
    ...
    def generate_samples(self):
        z = tf.constant(tf.random.normal(batch_c, dtype=tf.dtypes.float32))
...
class DatasetPipeline:
    ...
    def load_dataset(self):
        ...
        ds = ds.shuffle(50000, reshuffle_each_iteration=True)  # Перемешивает пароли
